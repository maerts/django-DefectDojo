...
class FortifyOnDemandParser(object):

    def get_scan_types(self):
        return ["Fortify on Demand Scan"]
    ...
    def format_mitigation(self, issue, meta_info) -> str:
        ...
        if category in meta_info and category:
            recommendation = meta_info[category].get("Recommendations")
            if recommendation:
                mitigation += "###Recommendation:\n {}\n".format(recommendation)

            tips = meta_info[category].get("Tips")
            if tips:
                mitigation += "###Tips:\n {}".format(tips)
        return mitigation
    ...
    def format_description(self, issue, meta_info) -> str:
        ...
        if len(issue) > 0:
            ...
            if category:
                ...
                if filename and linestart and snippet:
                    desc += "###Snippet:\n**File: {}: {}**\n```\n{}\n```\n".format(filename, linestart, snippet)
                ...
                if category in meta_info:
                    explanation = meta_info[category].get("Explanation")
                    if explanation:
                        desc += "##Explanation:\n {}".format(explanation)
        return desc
